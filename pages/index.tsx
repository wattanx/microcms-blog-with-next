...
export async function getStaticProps(context: GetStaticPropsContext) {
  ...
  const blogs = (await axios.get(`https://${config.serviceId}.microcms.io/api/v1/blog?limit=${limit}${
    categoryId === undefined ? '' : `filters=category[equals]${categoryId}`
  }&offset=${(page - 1) * limit}`, { headers: { 'X-API-KEY': config.apiKey } })).data;
  ...
  const selectedCategory = categoryId !== undefined ? categories.contents.find((content) => content.id === categoryId) : undefined;
  return {
    props: {
      blogs: blogs,
